...
- 当前机器已能通过 Git for Windows 使用 `patch`，但 `pkg` 预编译基础镜像下载曾返回 502，回退源码构建后仍缺少 `NASM`；exe 产物仍需在满足工具链的环境中验证。
...
| 8 | T21 | P1 | C2 | 完成可执行软件打包产物验证 | 打包配置已加入，当前机器已有 `patch`，但源码构建仍缺少 `NASM` |
...
- 当前机器打包时预编译 Node 基础镜像下载曾返回 502，回退源码构建后缺少 `NASM`，因此尚未生成 `dist/bilibiliwith163.exe`。
...
- 在安装 `NASM` 和必要编译工具，或能下载预编译 Node 基础镜像的环境中，`npm run build:exe` 成功生成 `dist/bilibiliwith163.exe`。
- exe 能读取同目录 `.env`。
