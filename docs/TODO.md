...
| --- | --- | --- | --- | --- | --- |
| 1 | T19 | P0 | C2 | 增加第三方引用入口 | 满足“能够被第三方程序引用”的交付要求，避免 `require()` 直接启动服务 |
| 2 | T20 | P0 | C1 | 创建 GitHub 仓库并推送首个公开分支 | 上传动作需要在前置安全检查和 README 完成后执行 |
| 3 | T01 | P0 | C1 | 清理 `public/style.css` 多段最终覆盖 | 直接影响后续 UI 可维护性和样式定位成本 |
| 4 | T06 | P1 | C2 | 提取前后端共享外观 schema | 新增外观字段需要同步三处，已出现维护成本 |
| 5 | T07 | P1 | C2 | 明确并收敛壁纸/控制台设置能力 | 当前接口保留但主流程未使用，容易误导后续开发 |
| 6 | T08 | P1 | C2 | 增加控制台手动点歌入口 | 服务端已有 API，补 UI 能提升主播控制效率 |
| 7 | T09 | P1 | C2 | B 站切房失败回滚与自动重连 | 提升直播稳定性，但涉及连接状态和 session 管理 |
| 8 | T10 | P1 | C2 | 网易云解析增加短期缓存和并发限制 | 降低弹幕峰值时外部 API 压力 |
| 9 | T11 | P1 | C2 | 音频下载增加超时、取消和 `.tmp` 清理 | 降低缓存异常导致的播放失败排查成本 |
| 10 | T21 | P1 | C2 | 可执行软件打包调研与可选实现 | 如果后续更倾向“可编译为可执行软件”，再引入 pkg/nexe/electron 等方案 |
| 11 | T12 | P2 | C2 | 处理 OBS 源错过 `player:play` 的同步问题 | 重连/刷新边界问题，完成后提升容错 |
| 12 | T13 | P2 | C3 | 用构建流程分离静态资源 | 长期方向，需先稳定前端模块边界 |
| 13 | T14 | P2 | C2 | 补充不依赖外网的单元/集成测试 | 核心行为稳定后逐步补测试护栏 |
| 14 | T15 | P2 | C2 | 增加音频缓存持久化索引 | 服务重启后复用缓存，但当前不是主链路阻塞 |
