...
| --- | --- | --- | --- | --- | --- |
| 1 | T18 | P0 | C2 | 重写根目录开源版 `README.md` | 正式开源前必须让第三方能独立理解、安装、运行和评估功能 |
| 2 | T19 | P0 | C2 | 增加第三方引用入口 | 满足“能够被第三方程序引用”的交付要求，避免 `require()` 直接启动服务 |
| 3 | T20 | P0 | C1 | 创建 GitHub 仓库并推送首个公开分支 | 上传动作需要在前置安全检查和 README 完成后执行 |
| 4 | T01 | P0 | C1 | 清理 `public/style.css` 多段最终覆盖 | 直接影响后续 UI 可维护性和样式定位成本 |
...
| 11 | T21 | P1 | C2 | 可执行软件打包调研与可选实现 | 如果后续更倾向“可编译为可执行软件”，再引入 pkg/nexe/electron 等方案 |
