...
- 已加入 Windows exe 打包脚本：`build:exe:caxa` 已能生成并启动验证自解压 exe；`@yao-pkg/pkg` 路线保留为高级备选。
...
- `@yao-pkg/pkg` 预编译基础镜像下载曾返回 502，回退源码构建后仍缺少 `NASM`；当前已用 `caxa` 简单路线绕过该问题并生成可启动 exe。
...
| 8 | T21 | P1 | C2 | 完善可执行软件发布包 | `caxa` exe 已生成并启动验证，后续需要补 Release 包结构和用户说明 |
| 9 | T22 | P1 | C1 | 验证自动安装和本机 `mpv` 播放链路 | 当前代码已接入播放器自动安装，但尚未实际下载和出声验证 |
| 10 | T13 | P2 | C3 | 用构建流程分离静态资源 | 长期方向，需先稳定前端模块边界 |
| 11 | T14 | P2 | C2 | 补充不依赖外网的单元/集成测试 | 核心行为稳定后逐步补测试护栏 |
| 12 | T15 | P2 | C2 | 增加音频缓存持久化索引 | 服务重启后复用缓存，但当前不是主链路阻塞 |

## 4. 任务拆解

### T21 P1/C2：完善可执行软件发布包

当前问题：

- `@yao-pkg/pkg` 路线仍受预编译基础镜像和本地源码构建工具链影响。
- 已新增 `npm run build:exe:caxa` 简单路线，并生成 `dist/bilibiliwith163-caxa.exe`。
- 当前 exe 能在独立目录读取 `.env` 并启动 HTTP 服务，但还需要整理面向普通用户的发布包。
...
- `npm run build:exe:caxa` 稳定生成 `dist/bilibiliwith163-caxa.exe`。
- exe 能读取启动目录 `.env`。
- exe 能加载内置 `public/` 和 `pic/`，也能在启动目录存在外部 `public/` 或 `pic/` 时优先读取外部目录。
- 发布包包含 exe、`.env.example`、必要说明，并明确 `.cache/` 是运行期目录。
- README 保持源码运行、`caxa` exe 运行和 `pkg` 高级路线差异说明。
