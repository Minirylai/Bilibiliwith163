...
| 2 | T03 | P1 | C1 | 修复点歌冷却扣减时机和队列上限语义 | 影响观众体验和配置理解，属于核心点歌链路 |
| 3 | T04 | P1 | C1 | 抽取前端通用工具函数 | 降低 `public/app.js` 与 `public/dashboard.js` 重复维护 |
| 4 | T05 | P1 | C1 | 共享 B 站解析辅助逻辑 | 降低调试脚本和正式模块协议适配不同步风险 |
| 5 | T06 | P1 | C2 | 提取前后端共享外观 schema | 新增外观字段需要同步三处，已出现维护成本 |
| 6 | T07 | P1 | C2 | 明确并收敛壁纸/控制台设置能力 | 当前接口保留但主流程未使用，容易误导后续开发 |
| 7 | T08 | P1 | C2 | 增加控制台手动点歌入口 | 服务端已有 API，补 UI 能提升主播控制效率 |
| 8 | T09 | P1 | C2 | B 站切房失败回滚与自动重连 | 提升直播稳定性，但涉及连接状态和 session 管理 |
| 9 | T10 | P1 | C2 | 网易云解析增加短期缓存和并发限制 | 降低弹幕峰值时外部 API 压力 |
| 10 | T11 | P1 | C2 | 音频下载增加超时、取消和 `.tmp` 清理 | 降低缓存异常导致的播放失败排查成本 |
