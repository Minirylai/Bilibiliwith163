...
- 已加入 Windows exe 打包脚本、`@yao-pkg/pkg` 开发依赖、资源打包配置和 `runtimePaths` 路径适配。

### 1.2 关键缺口

- 当前仓库没有 GitHub remote、CI 和 Release 检查清单。
- 当前机器未安装 GitHub CLI，且没有 `GH_TOKEN` / `GITHUB_TOKEN`，T20 无法自动创建 GitHub 仓库。
- 当前机器缺少 `patch` 命令，`pkg` 在无法使用预编译 Node 基础镜像时不能完成本地源码构建；exe 产物仍需在满足工具链的环境中验证。
...
| --- | --- | --- | --- | --- | --- |
| 1 | T20 | P0 | C1 | 创建 GitHub 仓库并推送首个公开分支 | 上传动作需要在前置安全检查和 README 完成后执行 |
| 2 | T01 | P0 | C1 | 清理 `public/style.css` 多段最终覆盖 | 直接影响后续 UI 可维护性和样式定位成本 |
| 3 | T06 | P1 | C2 | 提取前后端共享外观 schema | 新增外观字段需要同步三处，已出现维护成本 |
| 4 | T07 | P1 | C2 | 明确并收敛壁纸/控制台设置能力 | 当前接口保留但主流程未使用，容易误导后续开发 |
| 5 | T08 | P1 | C2 | 增加控制台手动点歌入口 | 服务端已有 API，补 UI 能提升主播控制效率 |
| 6 | T09 | P1 | C2 | B 站切房失败回滚与自动重连 | 提升直播稳定性，但涉及连接状态和 session 管理 |
| 7 | T10 | P1 | C2 | 网易云解析增加短期缓存和并发限制 | 降低弹幕峰值时外部 API 压力 |
| 8 | T11 | P1 | C2 | 音频下载增加超时、取消和 `.tmp` 清理 | 降低缓存异常导致的播放失败排查成本 |
| 9 | T21 | P1 | C2 | 完成可执行软件打包产物验证 | 打包配置已加入，但当前机器缺少 `patch`，尚未生成可验证 exe |
| 10 | T12 | P2 | C2 | 处理 OBS 源错过 `player:play` 的同步问题 | 重连/刷新边界问题，完成后提升容错 |
| 11 | T13 | P2 | C3 | 用构建流程分离静态资源 | 长期方向，需先稳定前端模块边界 |
| 12 | T14 | P2 | C2 | 补充不依赖外网的单元/集成测试 | 核心行为稳定后逐步补测试护栏 |
| 13 | T15 | P2 | C2 | 增加音频缓存持久化索引 | 服务重启后复用缓存，但当前不是主链路阻塞 |
...
### T21 P1/C2：完成可执行软件打包产物验证

当前问题：

- 已选择 `@yao-pkg/pkg`，并已加入 `npm run build:exe`、资源配置和运行路径适配。
- 当前机器打包时无法使用预编译 Node 基础镜像，回退源码构建后缺少 `patch` 命令，因此尚未生成 `dist/bilibiliwith163.exe`。
...
- 在安装 `patch` 和必要编译工具，或能下载预编译 Node 基础镜像的环境中，`npm run build:exe` 成功生成 `dist/bilibiliwith163.exe`。
- exe 能读取同目录 `.env`。
- exe 能加载内置 `public/` 和 `pic/`，也能在同目录存在外部 `public/` 或 `pic/` 时优先读取外部目录。
- README 保持源码运行和可执行运行差异说明。
